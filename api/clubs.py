...
@docs(
    tags=["Clubs"],
    summary="Локальные достижение для клубов",
    description="Локальные достижения для клубов.\nДля доступа требуется Bearer-токен в заголовке Authorization",
    responses={
        200: {"description": "Локальные достижения клубов", "schema": sh.AchievementsGlobalReturnSchema(many=True)},
        400: {"description": "Отсутствует один из параметров", "schema": sh.Error400Schema},
        401: {"description": "Авторизация не выполнена или авторизация выполнена от участника клуба не являющегося админом"},
        403: {"description": "Единственный администратор не может покинуть клуб."},
        500: {"description": "Server-side error (Ошибка на стороне сервера)"}
...
@request_schema(sh.AchievementsLocalSchema)
@validate.validate(validate.Achievements_local)
async def achievements_local(request: web.Request, parsed: validate.Achievements_local) -> web.Response:
    try:
        user_id = await core.check_authorization(request)
        if not isinstance(user_id, int):
            return user_id
        
        result = await func.achievements_local(parsed.club_id)
